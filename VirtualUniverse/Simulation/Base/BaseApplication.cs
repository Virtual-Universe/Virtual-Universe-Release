...
using VirtualUniverse.Framework.Configuration;
...
namespace VirtualUniverse.Simulation.Base
{
    /// <summary>
    ...
    public class BaseApplication
    {
        ...
        {
            bool VirtualUniverse_log = (File.Exists(Path.Combine(Util.configDir(), "VirtualUniverse.log")));
            bool VirtualUniverse_Server_log = (File.Exists(Path.Combine(Util.configDir(), "VirtualUniverse.Server.log")));
            bool isVirtualUniverseExe = System.AppDomain.CurrentDomain.FriendlyName == "VirtualUniverse.exe" ||
                               System.AppDomain.CurrentDomain.FriendlyName == "VirtualUniverse.vshost.exe";

            if (requested || !(isVirtualUniverseExe
                                   ? VirtualUniverse_log
                                   : VirtualUniverse_Server_log))
            {
                string resp = "no";
                if (!requested)
                {
                    ...
                    Console.WriteLine(
                        "\n\n   This is your first time running Virtual Universe, if not and you already configured your " +
                        "*ini.example files, please ignore this warning and press enter; " +
                        "Otherwise type yes and Virtual Universe will guide you trough configuration files.\n\nRemember, " +
                        "these file names are Case Sensitive in Linux and Proper Cased.\n1. ./VirtualUniverse.ini\nand\n2. " +
                    ...
                    Console.ResetColor();
                    resp = ReadLine("Do you want to configure Virtual Universe now", resp);
                }
                else
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                ...
                if (resp == "yes")
                {
                    ...
                    string dbPasswd = "virtualuniverse";
                    string dbSchema = "virtualuniverse";
                    string dbUser = "virtualuniverse";
                    ...
                    if (isStandalone)
                    {
                        string folder = isVirtualUniverseExe ? "Configuration/" : "VirtualUniverseServerConfiguration/";
                        MakeSureExists(folder + "Data/Data.ini");
                        IniConfigSource data_ini = new IniConfigSource(folder + "Data/Data.ini",
                                                                       Nini.Ini.IniFileType.VirtualUniverseStyle);
                        ...
                        conf = data_ini.AddConfig("VirtualUniverseConnectors");
                    ...
                    if (isVirtualUniverseExe)
                    {
                        MakeSureExists("VirtualUniverse.ini");
                        IniConfigSource VirtualUniverse_ini = new IniConfigSource("VirtualUniverse.ini", Nini.Ini.IniFileType.VirtualUniverseStyle);
                        IniConfigSource VirtualUniverse_ini_example = new IniConfigSource("VirtualUniverse.ini.example", Nini.Ini.IniFileType.VirtualUniverseStyle);

                        foreach (IConfig config in virtualuniverse_ini_example.Configs)
                        {
                            IConfig newConfig = virtualuniverse_ini.AddConfig(config.Name);
                            foreach (string key in config.GetKeys())
                            {
                        ...
                        virtualuniverse_ini.Save();
                        Console.ForegroundColor = ConsoleColor.Green;
                        Console.WriteLine("Your VirtualUniverse.ini has been successfully configured");
                        Console.ResetColor();
                        ...
                        if (isStandalone)
                            ...
                            conf.Set("Include-Grid", "Configuration/Grid/VirtualUniverseGridCommon.ini");
                        ...
                        {
                            MakeSureExists("Configuration/Grid/VirtualUniverseGridCommon.ini");
                            IniConfigSource grid_ini = new IniConfigSource("Configuration/Grid/VirtualUniverseGridCommon.ini",
                                                                           Nini.Ini.IniFileType.VirtualUniverseStyle);
                    ...
                    if (!isVirtualUniverseExe)
                    {
                        MakeSureExists("VirtualUniverseServerConfiguration/Login.ini");
                        IniConfigSource login_ini = new IniConfigSource("VirtualUniverseServerConfiguration/Login.ini",
                                                                        Nini.Ini.IniFileType.VirtualUniverseStyle);
                        IniConfigSource login_ini_example =
                            new IniConfigSource("VirtualUniverseServerConfiguration/Login.ini.example",
                                                Nini.Ini.IniFileType.VirtualUniverseStyle);
                        ...
                        MakeSureExists("VirtualUniverseServerConfiguration/GridInfoService.ini");
                        IniConfigSource grid_info_ini =
                            new IniConfigSource("VirtualUniverseServerConfiguration/GridInfoService.ini",
                                                Nini.Ini.IniFileType.VirtualUniverseStyle);
                    ...
                    if (isStandalone)
                    {
                        Console.WriteLine("\nYour loginuri is ");
                        Console.ForegroundColor = ConsoleColor.Cyan;
                        Console.WriteLine("http://" + gridIPAddress + (isVirtualUniverseExe ? ":9000/" : ":8002/"));
                        Console.ResetColor();
                    }
                    else
                    {
