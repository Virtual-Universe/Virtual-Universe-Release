...
namespace VirtualUniverse.Simulation.Base
{
    public class BinMigratorService
    {
        ...
        {
            if (!File.Exists("VirtualUniverse.version"))
                return 0;
            string file = File.ReadAllText("VirtualUniverse.version");
            return int.Parse(file);
        }

        public void SetBinVersion(int version)
        {
            File.WriteAllText("VirtualUniverse.version", version.ToString());
        }
        ...
        {
            if (File.Exists("VirtualUniverse.UserServer.exe"))
                File.Delete("VirtualUniverse.UserServer.exe");
        }
    ...
    public class IniMigrator
    {
                                         ...
                                         MigratorAction[] actions)
        {
            ...
            {
                IniConfigSource doc = new IniConfigSource(fileName, IniFileType.VirtualUniverseStyle);
