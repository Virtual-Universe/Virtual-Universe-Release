...
{
    public class VV_Api : MarshalByRefObject, IScriptApi
    {
        ...
        #region IVV_Api Members

        public void vvSetCloudDensity(LSL_Float density)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Moderate, "vvSetCloudDensity", m_host, "AA", m_itemID))
                return;
            if (!World.Permissions.CanIssueEstateCommand(m_host.OwnerID, false))
            {
                ShoutError("It appears that you do not have estate permissions here.");
                return;
            }
        ...
        public void vvUpdateDatabase(LSL_String key, LSL_String value, LSL_String token)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Moderate, "vvUpdateDatabase", m_host, "AA", m_itemID))
                return;
            AssetConnector.UpdateLSLData(token.m_string, key.m_string, value.m_string);
        }

        public LSL_List vvQueryDatabase(LSL_String key, LSL_String token)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Moderate, "vvQueryDatabase", m_host, "VV", m_itemID))
                return new LSL_List();
        ...
        public LSL_String vvSerializeXML(LSL_List keys, LSL_List values)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Moderate, "vvSerializeXML", m_host, "VV", m_itemID))
        ...
        public LSL_List vvDeserializeXMLKeys(LSL_String xmlFile)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Moderate, "vvDeserializeXMLKeys", m_host, "VV", m_itemID))
                return new LSL_List();
            XmlDocument doc = new XmlDocument();
        ...
        public LSL_List vvDeserializeXMLValues(LSL_String xmlFile)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Moderate, "vvDeserializeXMLValues", m_host, "VV",
        ...
        public void vvSetConeOfSilence(LSL_Float radius)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Low, "vvSetConeOfSilence", m_host, "VV", m_itemID))
                return;
            if (World.Permissions.IsGod(m_host.OwnerID))
                m_host.SetConeOfSilence(radius.value);
            else
                ShoutError("We apologize but it appears that you do not have god permissions here.");
        }

        public void vvJoinCombatTeam(LSL_Key uuid, LSL_String team)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Low, "vvJoinCombatTeam", m_host, "vv", m_itemID)) return;
            UUID avID;
            if (UUID.TryParse(uuid, out avID))
            {
                ...
                if (SP != null)
                {
                    ...
                    if (CP != null)
                    {
                        ...
                        {
                            SP.ControllingClient.SendAlertMessage("We apologize but it appears that you cannot join this team.");
                            return;
                        }
        ...
        public void vvLeaveCombat(LSL_Key uuid)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Low, "vvLeaveCombat", m_host, "VV", m_itemID)) return;
            UUID avID;
            if (UUID.TryParse(uuid, out avID))
            {
                IScenePresence SP = World.GetScenePresence(avID);
        ...
        public void vvJoinCombat(LSL_Key uuid)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Low, "vvJoinCombat", m_host, "VV", m_itemID)) return;
            UUID avID;
            if (UUID.TryParse(uuid, out avID))
            {
                IScenePresence SP = World.GetScenePresence(avID);
        ...
        public LSL_Float vvGetHealth(LSL_Key uuid)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvGetHealth", m_host, "VV", m_itemID))
        ...
        public LSL_String vvGetTeam(LSL_Key uuid)
        {
            UUID avID;
            if (UUID.TryParse(uuid, out avID))
            {
                if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Low, "vvGetTeam", m_host, "VV", m_itemID))
                    return new LSL_String();
                IScenePresence SP = World.GetScenePresence(avID);
                if (SP != null)
                {
            ...
            return "We apologize but there appears to be no team";
        }

        public LSL_List vvGetTeamMembers(LSL_String team)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Low, "vvGetTeamMembers", m_host, "VV", m_itemID))
                return new LSL_List();
        ...
        public LSL_String vvGetLastOwner()
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvGetLastOwner", m_host, "VV", m_itemID))
                return new LSL_String();
        ...
        public void vvSayDistance(int channelID, LSL_Float Distance, string text)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.VeryLow, "vvSayDistance", m_host, "VV", m_itemID))
                return;
        ...
        public void vvSayTo(string userID, string text)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Low, "vvSayTo", m_host, "VV", m_itemID)) return;
        ...
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvGetText", m_host, "VV", m_itemID))
                return new LSL_String();
            return m_host.Text;
        }

        public LSL_Rotation vvGetTextColor()
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvGetTextColor", m_host, "VV", m_itemID))
                return new LSL_Rotation();
            LSL_Rotation v = new LSL_Rotation(m_host.Color.R, m_host.Color.G, m_host.Color.B, m_host.Color.A);
            return v;
        }

        public void vvRaiseError(string message)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvRaiseError", m_host, "VV", m_itemID)) return;
            m_ScriptEngine.PostScriptEvent(m_itemID, m_host.UUID, "on_error", new object[] { message });
            throw new EventAbortException();
        }

        public void vvFreezeAvatar(string ID)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Moderate, "vvFreezeAvatar", m_host, "VV", m_itemID))
                return;
            ...
            if (UUID.TryParse(ID, out AgentID))
            {
                ...
                if (World.TryGetScenePresence(AgentID, out SP))
                {
                    ...
                    if (!ScriptProtection.CheckThreatLevel(ThreatLevel.High, "vvFreezeAvatar", m_host, "VV", m_itemID))
                        return;
                    SP.AllowMovement = false;
                }
            }
        ...
        public void vvThawAvatar(string ID)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.Moderate, "vvThawAvatar", m_host, "VV", m_itemID))
                return;
            UUID AgentID = UUID.Zero;
            if (UUID.TryParse(ID, out AgentID))
            {
                ...
                if (World.TryGetScenePresence(AgentID, out SP))
                {
                    ...
                    if (!ScriptProtection.CheckThreatLevel(ThreatLevel.High, "vvThawAvatar", m_host, "VV", m_itemID))
                        return;
                    SP.AllowMovement = true;
                }
                else
        ...
        public void vvRequestCombatPermission(string ID)
        {
            if (
                !ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvRequestCombatPermission", m_host, "VV", m_itemID))
                return;
            IScenePresence SP;
            UUID AgentID = UUID.Zero;
            if (UUID.TryParse(ID, out AgentID))
        ...
        public LSL_Integer vvGetWalkDisabled(string vPresenceId)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvGetWalkDisabled", m_host, "VV", m_itemID))
            ...
            if (item.PermsGranter != UUID.Zero)
            {
                ...
                if (presence != null)
                {
                    if ((item.PermsMask & ScriptBaseClass.PERMISSION_TAKE_CONTROLS) != 0)
                    {
                        ...
                        LSLError("We apologize but it appears that you do not have permission to access this information.");
                }
                else
                    LSLError("Agent does not exist in the sim.");
            }
        ...
        public void vvSetWalkDisabled(string vPresenceId, bool vbValue)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvSetWalkDisabled", m_host, "VV", m_itemID))
                return;
            TaskInventoryItem item;
        ...
        public LSL_Integer vvGetFlyDisabled(string vPresenceId)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvGetFlyDisabled", m_host, "VV", m_itemID))
                return new LSL_Integer();
            TaskInventoryItem item;
        ...
        public void vvSetFlyDisabled(string vPresenceId, bool vbValue)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvSetFlyDisabled", m_host, "VV", m_itemID))
                return;
            TaskInventoryItem item;
        ...
        public LSL_Key vvAvatarFullName2Key(string fullname)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "vvAvatarFullName2Key", m_host, "VV", m_itemID))
                return new LSL_String();
        ...
        public void vvSetEnv(LSL_String name, LSL_List value)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.VeryHigh, "vvSetEnv", m_host, "VV", m_itemID))
                return;
            if (!World.Permissions.IsGod(m_host.OwnerID))
            {
                LSLError("We apologize but it appears that you do not have god permissions.");
                return;
            }
        ...
        public void vvSetCharacterStat(string UUIDofAv, string StatName, float statValue)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "VVSetCharacterStat", m_host, "VV", m_itemID))
                return;
            ...
            if (presence != null)
            {
                ...
                LSLError("We apologize but it appears that the agent does not exist in the sim.");
        }

        public LSL_Integer vvGetIsInfiniteRegion()
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "VVGetIsInfiniteRegion", m_host, "VV", m_itemID))
                return 0;
            return new LSL_Integer(World.RegionInfo.InfiniteRegion ? 1 : 0);
        }

        public void vvAllRegionInstanceSay(LSL_Integer channelID, string text)
        {
            if (!ScriptProtection.CheckThreatLevel(ThreatLevel.None, "VVAllRegionInstanceSay", m_host, "VV", m_itemID))
                return;
        ...
        public LSL_List vvWindlightGetScene(LSL_List rules)
        {
        ...
        public LSL_List vvWindlightGetScene(int dayCycleIndex, LSL_List rules)
        {
            IEnvironmentSettingsModule environmentSettings = World.RequestModuleInterface<IEnvironmentSettingsModule>();
            if (environmentSettings == null)
                return new LSL_List(new object[2] { ScriptBaseClass.WL_ERROR, ScriptBaseClass.WL_ERROR_NO_SCENE_SET });
            ...
            if (cycle.Cycle.IsStaticDayCycle)
                return
                    new LSL_List(new object[2] { ScriptBaseClass.WL_ERROR, ScriptBaseClass.WL_ERROR_SCENE_MUST_NOT_BE_STATIC });
        ...
        public LSL_Integer vvWindlightGetSceneIsStatic()
        {
            IEnvironmentSettingsModule environmentSettings = World.RequestModuleInterface<IEnvironmentSettingsModule>();
            if (environmentSettings == null)
        ...
        public LSL_Integer vvWindlightGetSceneDayCycleKeyFrameCount()
        {
            IEnvironmentSettingsModule environmentSettings = World.RequestModuleInterface<IEnvironmentSettingsModule>();
            if (environmentSettings == null)
                return new LSL_Integer(-1);
        ...
        public LSL_List vvWindlightGetDayCycle()
        {
            IEnvironmentSettingsModule environmentSettings = World.RequestModuleInterface<IEnvironmentSettingsModule>();
            if (environmentSettings == null)
                return new LSL_List(new object[2] { ScriptBaseClass.WL_ERROR, ScriptBaseClass.WL_ERROR_NO_SCENE_SET });
        ...
        public LSL_Integer vvWindlightRemoveDayCycleFrame(int dayCycleFrame)
        {
            IEnvironmentSettingsModule environmentSettings = World.RequestModuleInterface<IEnvironmentSettingsModule>();
            if (environmentSettings == null)
        ...
        public LSL_Integer vvWindlightAddDayCycleFrame(LSL_Float dayCyclePosition, int dayCycleFrameToCopy)
        {
            IEnvironmentSettingsModule environmentSettings = World.RequestModuleInterface<IEnvironmentSettingsModule>();
            if (environmentSettings == null)
                return LSL_Integer.FALSE;
        ...
        public LSL_Integer vvWindlightSetScene(LSL_List list)
        {
            IEnvironmentSettingsModule environmentSettings = World.RequestModuleInterface<IEnvironmentSettingsModule>();
            if (environmentSettings == null)
        ...
        public LSL_Integer vvWindlightSetScene(int dayCycleIndex, LSL_List list)
        {
            IEnvironmentSettingsModule environmentSettings = World.RequestModuleInterface<IEnvironmentSettingsModule>();
            if (environmentSettings == null)
                return ScriptBaseClass.WL_ERROR_NO_SCENE_SET;
